...
type ApiSummaryOutageCheckidGetRequest struct {
	...
	from *int32
	to *int32
	order *SummaryOutageOrder
}

// Start time of period. Format is UNIX timestamp. Default value is one week earlier than &#x60;to&#x60;.
func (r ApiSummaryOutageCheckidGetRequest) From(from int32) ApiSummaryOutageCheckidGetRequest {
	r.from = &from
	return r
}

// End time of period. Format is UNIX timestamp. Default value is the current time.
func (r ApiSummaryOutageCheckidGetRequest) To(to int32) ApiSummaryOutageCheckidGetRequest {
	r.to = &to
	return r
}

// Sorting order of outages. Ascending or descending.
func (r ApiSummaryOutageCheckidGetRequest) Order(order SummaryOutageOrder) ApiSummaryOutageCheckidGetRequest {
	r.order = &order
	return r
}

func (r ApiSummaryOutageCheckidGetRequest) Execute() (*SummaryOutageRespAttrs, *http.Response, error) {
	return r.ApiService.SummaryOutageCheckidGetExecute(r)
}
...
func (a *SummaryOutageAPIService) SummaryOutageCheckidGetExecute(r ApiSummaryOutageCheckidGetRequest) (*SummaryOutageRespAttrs, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *SummaryOutageRespAttrs
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "SummaryOutageAPIService.SummaryOutageCheckidGet")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/summary.outage/{checkid}"
	...
	if r.order != nil {
		...
		var defaultValue SummaryOutageOrder = "asc"
