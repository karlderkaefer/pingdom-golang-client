...
// checks if the HTTPCustom type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &HTTPCustom{}
...
func (o *HTTPCustom) UnmarshalJSON(data []byte) (err error) {
	...
	varHTTPCustom := _HTTPCustom{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varHTTPCustom)

	if err != nil {
		return err
	}

	*o = HTTPCustom(varHTTPCustom)
...
func NewNullableHTTPCustom(val *HTTPCustom) *NullableHTTPCustom {
	return &NullableHTTPCustom{value: val, isSet: true}
}
