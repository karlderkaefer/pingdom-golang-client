...
// checks if the UpdateTeam type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &UpdateTeam{}
...
func (o *UpdateTeam) UnmarshalJSON(data []byte) (err error) {
	...
	varUpdateTeam := _UpdateTeam{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varUpdateTeam)

	if err != nil {
		return err
	}

	*o = UpdateTeam(varUpdateTeam)
...
func NewNullableUpdateTeam(val *UpdateTeam) *NullableUpdateTeam {
	return &NullableUpdateTeam{value: val, isSet: true}
}
