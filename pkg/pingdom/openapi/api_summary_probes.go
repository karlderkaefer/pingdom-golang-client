...
func (r ApiSummaryProbesCheckidGetRequest) From(from int32) ApiSummaryProbesCheckidGetRequest {
	r.from = &from
	return r
}

// End time of period. Format is UNIX timestamp. The defualt value is current time.
func (r ApiSummaryProbesCheckidGetRequest) To(to int32) ApiSummaryProbesCheckidGetRequest {
	r.to = &to
	return r
}

func (r ApiSummaryProbesCheckidGetRequest) Execute() (*SummaryProbesRespAttrs, *http.Response, error) {
	return r.ApiService.SummaryProbesCheckidGetExecute(r)
}
...
func (a *SummaryProbesAPIService) SummaryProbesCheckidGetExecute(r ApiSummaryProbesCheckidGetRequest) (*SummaryProbesRespAttrs, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *SummaryProbesRespAttrs
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "SummaryProbesAPIService.SummaryProbesCheckidGet")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/summary.probes/{checkid}"
