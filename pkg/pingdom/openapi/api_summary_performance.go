...
type ApiSummaryPerformanceCheckidGetRequest struct {
	...
	from *int32
	to *int32
	resolution *SummaryPerformanceResolution
	includeuptime *bool
	probes *string
	order *SummaryPerformanceOrder
}

// Start time of period. Format is UNIX timestamp. Default value is 10 intervals earlier than &#x60;to&#x60;.
func (r ApiSummaryPerformanceCheckidGetRequest) From(from int32) ApiSummaryPerformanceCheckidGetRequest {
	r.from = &from
	return r
}

// End time of period. Format is UNIX timestamp. Default value is the current time.
func (r ApiSummaryPerformanceCheckidGetRequest) To(to int32) ApiSummaryPerformanceCheckidGetRequest {
	r.to = &to
	return r
}

// Interval size
func (r ApiSummaryPerformanceCheckidGetRequest) Resolution(resolution SummaryPerformanceResolution) ApiSummaryPerformanceCheckidGetRequest {
	r.resolution = &resolution
	return r
}

// Include uptime information.
func (r ApiSummaryPerformanceCheckidGetRequest) Includeuptime(includeuptime bool) ApiSummaryPerformanceCheckidGetRequest {
...
func (r ApiSummaryPerformanceCheckidGetRequest) Probes(probes string) ApiSummaryPerformanceCheckidGetRequest {
	r.probes = &probes
	return r
}

// Sorting order of sub intervals. Ascending or descending.
func (r ApiSummaryPerformanceCheckidGetRequest) Order(order SummaryPerformanceOrder) ApiSummaryPerformanceCheckidGetRequest {
	r.order = &order
	return r
}

func (r ApiSummaryPerformanceCheckidGetRequest) Execute() (*SummaryPerformanceRespAttrs, *http.Response, error) {
	return r.ApiService.SummaryPerformanceCheckidGetExecute(r)
}
...
func (a *SummaryPerformanceAPIService) SummaryPerformanceCheckidGetExecute(r ApiSummaryPerformanceCheckidGetRequest) (*SummaryPerformanceRespAttrs, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *SummaryPerformanceRespAttrs
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "SummaryPerformanceAPIService.SummaryPerformanceCheckidGet")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/summary.performance/{checkid}"
	...
	if r.resolution != nil {
		...
		var defaultValue SummaryPerformanceResolution = "hour"
		r.resolution = &defaultValue
	}
	if r.includeuptime != nil {
		parameterAddToHeaderOrQuery(localVarQueryParams, "includeuptime", r.includeuptime, "")
	} else {
	...
	if r.order != nil {
		...
		var defaultValue SummaryPerformanceOrder = "asc"
