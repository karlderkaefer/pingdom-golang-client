...
func (r ApiSummaryAverageCheckidGetRequest) From(from int32) ApiSummaryAverageCheckidGetRequest {
	r.from = &from
	return r
}

// End time of period. Format is UNIX timestamp. Default is the current time
func (r ApiSummaryAverageCheckidGetRequest) To(to int32) ApiSummaryAverageCheckidGetRequest {
	r.to = &to
	return r
}
...
func (r ApiSummaryAverageCheckidGetRequest) Probes(probes string) ApiSummaryAverageCheckidGetRequest {
	r.probes = &probes
	return r
}

// Include uptime information
func (r ApiSummaryAverageCheckidGetRequest) Includeuptime(includeuptime bool) ApiSummaryAverageCheckidGetRequest {
	r.includeuptime = &includeuptime
	return r
}

// Split response times into country groups
func (r ApiSummaryAverageCheckidGetRequest) Bycountry(bycountry bool) ApiSummaryAverageCheckidGetRequest {
	r.bycountry = &bycountry
	return r
}

// Split response times into probe groups
func (r ApiSummaryAverageCheckidGetRequest) Byprobe(byprobe bool) ApiSummaryAverageCheckidGetRequest {
	r.byprobe = &byprobe
	return r
}

func (r ApiSummaryAverageCheckidGetRequest) Execute() (*SummaryRespAttrs, *http.Response, error) {
	return r.ApiService.SummaryAverageCheckidGetExecute(r)
}
...
func (a *SummaryAverageAPIService) SummaryAverageCheckidGetExecute(r ApiSummaryAverageCheckidGetRequest) (*SummaryRespAttrs, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *SummaryRespAttrs
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "SummaryAverageAPIService.SummaryAverageCheckidGet")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/summary.average/{checkid}"
	...
	if r.bycountry != nil {
		parameterAddToHeaderOrQuery(localVarQueryParams, "bycountry", r.bycountry, "")
	} else {
		var defaultValue bool = false
		r.bycountry = &defaultValue
	}
	if r.byprobe != nil {
		parameterAddToHeaderOrQuery(localVarQueryParams, "byprobe", r.byprobe, "")
	} else {
		var defaultValue bool = false
		r.byprobe = &defaultValue
	}
