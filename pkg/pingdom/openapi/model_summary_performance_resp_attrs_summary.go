...
func DaysAsSummaryPerformanceRespAttrsSummary(v *Days) SummaryPerformanceRespAttrsSummary {
	return SummaryPerformanceRespAttrsSummary{
		Days: v,
	}
}
...
func (dst *SummaryPerformanceRespAttrsSummary) UnmarshalJSON(data []byte) error {
	var err error
	match := 0
	// try to unmarshal data into Days
	err = newStrictDecoder(data).Decode(&dst.Days)
	if err == nil {
		jsonDays, _ := json.Marshal(dst.Days)
		if string(jsonDays) == "{}" { // empty struct
			dst.Days = nil
		} else {
			match++
		}
	...
	// try to unmarshal data into Hours
	err = newStrictDecoder(data).Decode(&dst.Hours)
	if err == nil {
		jsonHours, _ := json.Marshal(dst.Hours)
		if string(jsonHours) == "{}" { // empty struct
			dst.Hours = nil
		} else {
			match++
		}
	...
	// try to unmarshal data into Weeks
	err = newStrictDecoder(data).Decode(&dst.Weeks)
	if err == nil {
		jsonWeeks, _ := json.Marshal(dst.Weeks)
		if string(jsonWeeks) == "{}" { // empty struct
			dst.Weeks = nil
		} else {
			match++
		}
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("data matches more than one schema in oneOf(SummaryPerformanceRespAttrsSummary)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("data failed to match schemas in oneOf(SummaryPerformanceRespAttrsSummary)")
	}
}
...
func (src SummaryPerformanceRespAttrsSummary) MarshalJSON() ([]byte, error) {
	if src.Days != nil {
		return json.Marshal(&src.Days)
	}

	if src.Hours != nil {
		return json.Marshal(&src.Hours)
	}

	if src.Weeks != nil {
		return json.Marshal(&src.Weeks)
	}
...
type NullableSummaryPerformanceRespAttrsSummary struct {
	value *SummaryPerformanceRespAttrsSummary
	isSet bool
}

func (v NullableSummaryPerformanceRespAttrsSummary) Get() *SummaryPerformanceRespAttrsSummary {
	return v.value
}

func (v *NullableSummaryPerformanceRespAttrsSummary) Set(val *SummaryPerformanceRespAttrsSummary) {
	v.value = val
	v.isSet = true
}

func (v NullableSummaryPerformanceRespAttrsSummary) IsSet() bool {
	return v.isSet
}

func (v *NullableSummaryPerformanceRespAttrsSummary) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableSummaryPerformanceRespAttrsSummary(val *SummaryPerformanceRespAttrsSummary) *NullableSummaryPerformanceRespAttrsSummary {
	return &NullableSummaryPerformanceRespAttrsSummary{value: val, isSet: true}
}

func (v NullableSummaryPerformanceRespAttrsSummary) MarshalJSON() ([]byte, error) {
